...
async function run() {
  try {
    ...
    const database = client.db("Emloyee");
    const userCollection = database.collection("EmployeCollection");
    const PhoneBuyCollection
    = database.collection("PhoneBuyCollection");
...
app.get('/update/:id', async (req, res) => {
    ...
    app.post('/add', async (req, res) => {
      ...
      if(checkExistingData){
        return res.send({message:"Info already exists in the database"})
      }
    ...
    app.post('/', async (req, res) => {
      try {
        const newUser = req.body;
        // const existingData = await DataModel.findOne({ name: newUser.name, email: newUser.email });
        // if (!existingData) {
        //   // Data doesn't exist in the database, so insert it
        //   const result = await DataModel.create(newUser);
        //   console.log(`${result.insertedCount} documents were inserted`);
        //   res.json({ message: 'Data inserted successfully.' });
        // } else {
        //   // Data already exists, you can choose to update it if needed or skip the insertion
    ...
    app.put('/update/:id', async (req, res) => {
      ...
      const currentUser = {
        $set: {
          ...
          joiningDate:updateUser.joiningDate,
          joiningDate:updateUser.joiningDate,
